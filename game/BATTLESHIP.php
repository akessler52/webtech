...
<head lang="en">
   <meta charset="utf-8">
   <title>Battle Ship EXMAPLE</title>
   <style>
	.intro { 
		font-size:10pt; 
		font-style:italic 
...
function imagePreload() {
  ...
  for (i=0;i<ids.length;++i) {
    var img = new Image, name = "BS"+ids[i]+".png";
    img.src = name;
    preloaded[i] = img;
  }
  window.status = "";
...
function setImage(y,x,id,ispc) {
  if ( ispc ) {
    computer[y][x][0] = id;
    document.images["pc"+y+"_"+x].src = "BS"+id+".png";
  } else {
    player[y][x][0] = id;
    document.images["ply"+y+"_"+x].src = "BS"+id+".png";
  }
}
...
function showGrid(ispc) {
  ...
  for (y=0;y<gridy;++y) {
    for (x=0;x<gridx;++x) {
      if ( ispc )
      document.write ('<a href="javascript:gridClick('+y+','+x+');"><img name="pc'+y+'_'+x+'" src="BS100.png" width=36 height=36></a>');
      else
      document.write ('<a href="javascript:void(0);"><img name="ply'+y+'_'+x+'" src="BS'+player[y][x][0]+'.png" width=36 height=36></a>');
    }
    document.write('<br>');
  }
}
